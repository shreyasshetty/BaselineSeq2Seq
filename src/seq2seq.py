...
def share_embedding_attention_decoder(embedding, decoder_inputs, initial_state, attention_states,
                                      cell, num_symbols, embedding_size, num_heads=1,
                                      output_size=None, output_projection=None,
                                      feed_previous=False,
                                      update_embedding_for_previous=True,
...
def share_embedding_attention_seq2seq(encoder_inputs, decoder_inputs, cell,
                                      num_encoder_symbols, num_decoder_symbols,
                                      embedding_size,
  ...
  with variable_scope.variable_scope(scope or "embedding_attention_seq2seq"):
    ...
    encoder_outputs, encoder_state = rnn.rnn(
        cell, emb_encoder_inputs, dtype=dtype, scope=scope)
    ...
    if isinstance(feed_previous, bool):
      return share_embedding_attention_decoder(
          embedding, decoder_inputs, encoder_state, attention_states, cell,
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                         reuse=reuse):
        outputs, state = share_embedding_attention_decoder(
            embedding, decoder_inputs, encoder_state, attention_states, cell,
            num_decoder_symbols, embedding_size, num_heads=num_heads,
            output_size=output_size, output_projection=output_projection,
            feed_previous=feed_previous_bool,
